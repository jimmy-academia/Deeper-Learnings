...
class DialogDataset(Dataset):
    ...
    def __init__(self, data, padding=0,
                 ...
                 context_padded_len=50, option_padded_len=50, shuffle=True, last=False):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        # negative_indices = list(range(n_positive, n_positive+n_negative))
    ...
    def collate_fn(self, datas):
        ...
        if self.last:
            ...
            # build context
            batch['context_lens'] = [
                [min(max(len(opt), 1), self.context_padded_len)
                    for opt in data['context']]
                    for data in datas]
            padded_len = min(
                self.context_padded_len, 
                max(sum(batch['context_lens'], [])))

            batch_context_len = max([len(data['context']) for data in datas])

            batch['context'] = torch.tensor(
                        [[pad_to_len(opt, padded_len, self.padding) 
                            for opt in utt_to_len(data['context'], batch_context_len)]
                            for data in datas]
                    )
...
def utt_to_len(utts, max_length):
    return utts[:max_length] + [utts[-1]]*(max_length-len(utts))