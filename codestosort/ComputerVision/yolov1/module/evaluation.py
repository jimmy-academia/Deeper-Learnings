...
def evaluate(predict_dir, answer_dir):
    detpath = os.path.join(predict_dir, '{:s}.txt')
    annopath = os.path.join(answer_dir, '{:s}.txt')
    imagenames = [x.split('.')[0] for x in os.listdir(answer_dir) if x.endswith('.txt')]
    ...
    # print(detpath, annopath, len(imagenames))
